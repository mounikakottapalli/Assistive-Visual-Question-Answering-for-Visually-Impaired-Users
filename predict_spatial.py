...
def check_model_file_access(model_file):
    print(f"Current working directory: {os.getcwd()}")
    print(f"Trying to open model file at path: {os.path.abspath(model_file)}")

    if not os.path.exists(model_file):
        print(f"ERROR: File does not exist: {os.path.abspath(model_file)}")
        sys.exit(1)
    if not os.access(model_file, os.R_OK):
        print(f"ERROR: No read permission for file: {os.path.abspath(model_file)}")
        sys.exit(1)
...
if __name__ == "__main__":
    ...
    # --- DEBUG: Check model file access ---
    check_model_file_access(model_file)
